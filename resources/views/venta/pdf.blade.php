...
<head>
    ...
    <style>
       ...
       .table th, .table td {
        ...
        .table th:nth-child(1), .table td:nth-child(1){ width: 29%; }   /* Equipo (imagen) */
        .table th:nth-child(2), .table td:nth-child(2){ width: 35%; text-align:left; }  /* Descripción */
        .table th:nth-child(3), .table td:nth-child(3){ width: 8%; }     /* Cantidad */
        .table th:nth-child(4), .table td:nth-child(4){ width: 15%; }    /* Precio Unitario */
        .table th:nth-child(5), .table td:nth-child(5){ width: 15%; }    /* Subtotal */
...
@php
    ...
    // prioridad: columna tradein_total, luego valor_a_cuenta, luego valor (legacy)
    $tmpTradeinSum = $tradeins->sum('valor_a_cuenta');
    if (!$tmpTradeinSum) {
        $tmpTradeinSum = $tradeins->sum('valor');
    }
    $tradeinTotal  = (float)($venta->tradein_total ?? $tmpTradeinSum);

    // ✅ Mostrar trade-in SOLO si realmente hay registros y total > 0
    $mostrarTradeIn = ($tradeins->count() > 0) && ($tradeinTotal > 0);

    $totalNeto     = (float)($venta->total_neto ?? max($totalOriginal - ($mostrarTradeIn ? $tradeinTotal : 0), 0));
...
<table class="table">
    ...
    <tbody>
        ...
        @php
            ...
            $esRegalo = ($subLinea <= 0); // ✅ si 0 o menor: NO mostrar subtotal
            $imgRel = $item->producto->imagen ?? 'default.jpg';
            $imgPath = public_path('storage/' . ltrim($imgRel, '/'));
        @endphp
        <tr>
            <td>
                <img src="{{ $imgPath }}" width="50" alt="Imagen del producto">
            </td>
            ...
            <td>
                {{-- ✅ SUBTOTAL: si es 0 o menor, NO mostrar nada --}}
                @if(!$esRegalo && $subLinea > 0)
            ...
            <tbody>
                @foreach($tradeins as $ti)
                    @php
                        ...
                        $valorLinea  = (float)($ti->valor_a_cuenta ?? $ti->valor ?? 0);
                    @endphp
                    <tr>
                        <td>
...
<div class="total-box">
    @if((float)($venta->subtotal ?? 0) > 0)
        <p><strong>Subtotal:</strong> ${{ number_format((float)$venta->subtotal, 2) }}</p>
    @endif

    @if((float)($venta->descuento ?? 0) > 0)
        <p><strong>Descuento:</strong> ${{ number_format((float)$venta->descuento, 2) }}</p>
    @endif

    @if((float)($venta->envio ?? 0) > 0)
        <p><strong>Envío:</strong> ${{ number_format((float)$venta->envio, 2) }}</p>
    @endif

    @if((float)($venta->iva ?? 0) > 0)
        <p><strong>IVA:</strong> ${{ number_format((float)$venta->iva, 2) }}</p>
    @endif

    @if($mostrarTradeIn && $totalOriginal > 0)
    ...
    @elseif((float)($venta->total ?? 0) > 0)
        <p class="highlight"><strong>Total:</strong> ${{ number_format((float)$venta->total, 2) }}</p>
    @endif
...
@if($saldoRestante > 0.01)
    <p style="color:#b91c1c; font-weight:bold;">
         Saldo pendiente por liquidar: ${{ number_format($saldoRestante, 2) }} MXN
    </p>
@elseif($baseProgreso > 0)
    <p style="
        background:#eafaf4;
        ...
        font-size: 13px;
    ">
        Pago completado — Esta remisión ha sido liquidada en su totalidad. ¡Gracias por su puntualidad!
    </p>
@endif
</div>
...
<div style="page-break-before: always; margin-top: -0.5cm; font-family: Arial, sans-serif;">
    ...
    @else
        ...
        @php
            ...
            $pagosFinanciamiento = $venta->pagosFinanciamiento()
                ->with('pago')
                ->orderBy('fecha_pago')
                ->get();
            ...
            $montoInicial = $montoAnticipoReal;
            ...
            foreach ($pagosFinanciamiento as $pago) {
                ...
                $pagoRelacionado = $pago->pago;
                $estaPagadoCuota = $pagoRelacionado && $pagoRelacionado->aprobado;

                if (strtolower($desc) === 'pago inicial') {
                    $montoLinea = $pagoRelacionado
                        ? (float)$pagoRelacionado->monto
                        : (float)$pago->monto;
                    $montoInicial = $montoLinea;
                    $label = 'Anticipo';
                } else {
                    $label = $desc !== '' ? ucfirst($desc) : 'Pago';
                    $montoLinea = $estaPagadoCuota
                        ? (float)$pagoRelacionado->monto
                        : (float)$pago->monto;
                }

                $pagosTxt[] = "{$label} - {$fechaEs}: $" . number_format($montoLinea, 2, '.', ',');
            }
            ...
            $montoFinanciadoPlan = (float)$pagosFinanciamiento->sum('monto');

            $pagosPlanAprobados = $pagosFinanciamiento->filter(function($pf){
                return $pf->pago
                    && $pf->pago->aprobado
                    ...
                    <tr valign="top">
                        <td width="50%" style="padding-right: 15px;">
                            @foreach ($col1 as $linea)
                                <p style="margin:0 0 6px; color:#333;"><strong>{{ $linea }}</strong> hola</p>
                            @endforeach
                        </td>
                        <td width="50%">
        ...
        <div style="margin-bottom:1rem;">
            ...
            <table style="width: 100%; font-size: 13px; margin-top: 0.5rem;">
                ...
                <tr>
                    <td><strong>Anticipo (pagado o programado):</strong></td>
                    <td>${{ number_format($montoInicial, 2, '.', ',') }}</td>
                </tr>
                <tr>
                    <td><strong>Monto financiado (contrato):</strong></td>
                    <td>
                        ${{ number_format($montoFinanciadoBase, 2, '.', ',') }}
                    </td>
                </tr>
                <tr>
                ...
                @if($venta->plan === 'credito')
                    ...
                    <tr>
                        <td><strong>Total a pagar con intereses (programado):</strong></td>
                        <td>
