...
.desktop-filter select {
  ...
  font-weight: 500; cursor: pointer; min-width: 130px;
  color: #0f172a;
}
@media (max-width: 767.98px) {
...
<script>
  $(function () {
    ...
    // ===== Poblar select de agentes desde la tabla =====
const colAgente = 2; // índice columna "Usuario"
...
$('#filtroAgente').on('change', function () {
  ...
  if (!val) {
      tabla.column(colAgente).search('', true, false).draw();
      return;
  ...
  //  Escapar caracteres especiales de regex (punto, paréntesis, etc.)
  const escaped = normalize(val).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  
  // Búsqueda exacta en columna Usuario
...
<div class="hero blur-on-sheet fade-slide d-flex align-items-center justify-content-between flex-wrap" style="margin-top:20px;">
  ...
  <div class="desktop-filter">
    <i class="fa-solid fa-user-tie" style="opacity:.7"></i>
