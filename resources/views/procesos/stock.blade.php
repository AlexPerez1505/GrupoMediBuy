...
<div class="formulario-contenedor">
    ...
    <p class="card-text">Aquí puedes gestionar el proceso de actualización de stock del equipo con ID {{ $id }}.</p>

    <form id="formulario-stock" action="{{ route('procesos.guardar', $id) }}" method="POST" enctype="multipart/form-data">

        <input type="hidden" name="tipo_proceso" value="stock">

        <div class="form-group">
            <label for="descripcion" class="label_nomina">Descripción</label>
            <textarea class="form-control select" name="descripcion_proceso" placeholder="Describe los cambios en el stock." style="height: 130px;" required></textarea>
        </div>
...
<div class="form-group">
   ...
   <br>
   <div class="form-group">
    <label for="ficha_tecnica" class="label_nomina">
        
        Ficha Técnica
        <div class="info-icon">
            ?
            <div class="info-tooltip text-center">Seleccione una ficha técnica existente</div>
        </div>
    </label>
    <div class="d-flex align-items-start input-containerr">
    <select id="ficha_tecnica" name="ficha_tecnica_id" class="form-control">
            @if($fichas->count() > 0)
                ...
                @endforeach
            @else
                <p>No hay fichas técnicas disponibles.</p>
            @endif
        </select>
    </div>
</div>
...
function mostrarNombreArchivo(input) {
    ...
    if (input.files.length > 0) {
        fileInputText.textContent = "Archivo seleccionado";
    } else {
                    ...
                    if (xhr.status === 200) {
                        ...
                        if (response.errors) {
                            for (let campo in response.errors) {
                                errorHtml += `<li>${response.errors[campo]}</li>`;
                            }
                        } else {
                            errorHtml = `<li>${response.message || 'Ocurrió un error al guardar el proceso.'}</li>`;
                        }
