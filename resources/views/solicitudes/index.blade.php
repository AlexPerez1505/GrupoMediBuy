...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .wrap{max-width:1800px;margin:24px auto 64px;padding:0 24px}
  .card{background:var(--card);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden}

  /* Head */
  .head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px;border-bottom:1px solid var(--line); top:10px}
  ...
  /* Toolbar sticky (desktop y móvil) */
.toolbar-sticky {
  position: sticky;
  top: 70px;
  z-index: 50;
  ...
  border-radius: var(--radius) var(--radius) 0 0; /* solo redondea arriba */
  box-shadow: 0 2px 12px rgba(0,0,0,0.04);
  margin-bottom: 0; /* para que no haya espacio entre toolbar y card */
}
/* Ajustes para que las barras internas se integren */
.toolbar-sticky .bar {
  border-bottom: none; /* quitamos la línea inferior duplicada */
  border-radius: var(--radius) var(--radius) 0 0;

}


/* La .card principal pierde el borde superior (lo hereda el toolbar) */
.card-principal {
...
#contenedor-solicitudes {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(290px, 1fr)); /* columnas flexibles */
  gap: 16px;                     /* espacio entre tarjetas */
}

/* Anular los márgenes verticales de Bootstrap que desalinean el grid */
#contenedor-solicitudes > .card.my-2 {
  margin-top: 0 !important;
  margin-bottom: 0 !important;
}

/* El skeleton de carga debe ocupar todo el ancho, no una sola columna */
#skel {
  grid-column: 1 / -1;
}
</style>
